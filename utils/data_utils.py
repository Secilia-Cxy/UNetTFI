...
def get_training_idxs(df, len_seq_in, len_seq_predict, data_split, region, years):
    ...
    for i in range(df.shape[0] - len_seq_in - len_seq_predict):
        # date check
        ...
        if start_dt + timedelta(hours=hours_to_predict, minutes=45, seconds=0) == end_dt:
            ...
            in_seq = [i + j for j in range(len_seq_in + 1)]
            out_seq = [i + len_seq_in + j for j in range(len_seq_predict + 1)]
    ...
    for region in regions:
        ...
        for year in years:
            ...
            if product == "RATE":
                ...
                # data_numpy = np.array(ds)
                # total = data_numpy.size
                # for t in thresholds:
                #     data_true = data_numpy > t
                #     num_true = np.sum(data_true)
                #     print(f"threshold {t}, {num_true}/{total}, ratio : {num_true / total:.4f}")
                # f.close()
            else:
                ...
                ds = f["REFL-BT"]
                yearly_records[year] = ds
                # data_numpy = np.array(ds, dtype="float16")
                # # fig = plt.figure()
                # # plt.hist(data_numpy.view(-1))
                # # plt.show()
                # # plt.close()
                # print(np.histogram(data_numpy.reshape(-1)))
                # f.close()
...
def get_sequence(
    ...
    return np.array(prod_seq), mask_seq, year
...
def get_file(
    ...
    x, year = read_file(sample_id, ds, region)
    x = np.float32(x)
    if VERBOSE:
    ...
    if product == "RATE":
        ...
        # print(x.shape, np.min(x), np.max(x), np.mean(x))
        return x, masks, year
    else:
        raw_bands = ['IR_016', 'IR_039', 'IR_087', 'IR_097', 'IR_108', 'IR_120', 'IR_134', 'VIS006', 'VIS008', 'WV_062',
                     'WV_073']
        idx = [raw_bands.index(b) for b in bands]
        for j, b in zip(idx, bands):
            if preprocess is not None:
                x[j] = preprocess_HRIT(x[j], preprocess[b])
        return x[idx], masks, year
